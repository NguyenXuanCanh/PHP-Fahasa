        ...
        <div class="row ml-3 mt-2">
            ...
            <div class="col-9">
                <div class="row" id="renderListProductHere">
                    ...
                    if ($sortPrice == -1) {
                        $sql = "SELECT * from product where old = 1 order by price ASC limit " . $itemPerPage . " OFFSET " . $offset;
                    } else {
                        $sql = "SELECT * from product where old = 1 order by price DESC limit " . $itemPerPage . " OFFSET " . $offset;
                    }
                ...
                <div class="pageDivider text-center">
                    ...
                    <?php if ($pageCurrent > 1) { ?>
                        <span class="item" id="to-prev">
                            <a href="?type=pcsc&page=<?= $prevPage ?>">
                                < </a>
                        </span>
                    <?php } ?>
                    <?php for ($i = 1; $i <= $totalPage; $i++) { ?>
                        <?php if ($i != $pageCurrent) { ?>
                            <?php if ($i > $pageCurrent - 3 && $i < $pageCurrent + 3) { ?>
                                <span class="item active"><a href="?type=pcsc&page=<?= $i ?>"><?= $i ?></a></span>
                            <?php } ?>
                        <?php } else { ?>
                            <span class="item active"><a href="?type=pcsc&page=<?= $i ?>"><strong><?= $i ?></strong></a></span>
                        <?php } ?>
                    <?php } ?>
                    <?php if ($pageCurrent < $totalPage) {
                        $nextPage = $pageCurrent + 1;
                    ?>
                        <span class="item" id="to-next"><a href="?type=pcsc&page=<?= $nextPage ?>">></a></span>
                    <?php } ?>
                </div>
            </div>
        </div>
